...
class QuestionRendererTest < ActiveSupport::TestCase
  ...
  test "renders unlabeled code blocks with a generic title" do
    markdown_content = <<~MARKDOWN
      ```
      puts "Hello"
      ```
  ...
  test "formatter supports line numbers" do
    formatter = QuestionRenderer::CodeFormatter.new(line_numbers: true)
    tokens = Rouge::Lexers::Ruby.lex("puts 'Hello'\n")
    rendered = +""

    formatter.stream(tokens) do |chunk|
      rendered << chunk
    end

    assert_includes rendered, "<table"
    assert_includes rendered, "<td class=\"rouge-gutter gl\""
  end
